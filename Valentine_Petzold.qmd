...
power_function(4)(4)
```
A function calling another one.
...
g(5)
```
rm(z) z doesn't exist anymore.

### Vector 'interpolation' (broacasting)
...
Most operation in R are vectorized: the same operation is applied by default to all the elements of a vector. To ease that, vectors can be expanded to match a certain length, simply by copying the values of the vector a certain number of times. 

For instance, '1' can be interpreted as 'c(1,1,1,1)' if this is needed for a comnbination with a length 3 vector (as above). 

```{r}
x>=2
```
```{r}
set.seed(42) #ensure reproductability by starting the random number generator in state 42 (any integer works)
x<-runif(20, min=-1, max=1)#a random vector with a uniform distribution (mean=0)
x
sum(x>0)
sum(x>mean(x))
length(x[x>mean(x)])
...
```{r}
u<-c(1,2,3)
v<-c(4,5)# if we duplicate v ...
u+v
```
```{r}
u<-c(1,2,3,4)
v<-c(-1, 1) #need to duplicate v
u+v
u*v
```
...
where $p_k$ is the probability of observing $k$. A possible naive definition is the following one (see function above). 
```{r}
H<-function(p) { 
  -sum(p*log(p))
  }
...
```{r}
my_dist<-c(0, 0.5, 0.5)#distribution over 1, 2, 3
H(my_dist)
```
The natural convention in entropy calculation is that $p_k\log(p_k)=0$ when $p_k=0$. We can use 'ifelse' to im^plement that. 

```{r}
working_H<-function(p) { 
  p_logp <- ifelse(p>0, p*log(p), O) ## for element of p that >0, we use the formula (second parameter of ifelse) and we use 0 for the others (third parameter)
  -sum(p_logp)
  }
```
